...
def create_app():
    app = Flask(__name__)
    app.config["SQLALCHEMY_DATABASE_URI"] = 'postgresql://admin:admin@localhost:5432/parking_db'
    ...
    def create_parking():
        ...
        if count_places is None:
            return jsonify({"error": "Field 'count_places' must be an integer"}), 400
        
        if count_available_places is None:
            return jsonify({"error": "Field 'count_available_places' must be an integer"}), 400

        if count_available_places > count_places:
            return jsonify({"error": "Available places cannot exceed total places"}), 400
    ...
    def client_parkings():
        ...
        open_or_not = db.session.query(Parking.opened).filter(Parking.id == parking_id).scalar()
    ...
    def leaving_parking():
        ...
        if not client.credit_card:
                return jsonify({"error": f"There is no credit card info of user {client_id}"}), 400
        parking = db.session.get(Parking, parking_id)
        if not parking:
            return jsonify({"error": f"There is no parking {parking_id}"}), 400
